...
const Projects = () => {
  ...
  if (loading) return <p className="text-center text-white mt-20">Loading projects...</p>;
  ...
  return (
    <section id="projects" className="relative py-20 bg-black text-white overflow-hidden">
      ...
      <div
        ...
        style={{ backgroundImage: "url('https://i.postimg.cc/7PQQZLbb/projects-bg-1.png')" }}
      />
      <div
        className="absolute bottom-0 right-0 w-1/2 h-1/2 bg-cover bg-right-bottom z-0 opacity-30 pointer-events-none"
        style={{ backgroundImage: "url('https://i.postimg.cc/X7G1G0yL/projects-bg-2.png')" }}
      />

      <div className="relative z-10 mx-auto w-4/5">
        <h2 className="text-3xl sm:text-4xl font-bold text-center text-red-500 mb-2">{title}</h2>
        <div className="h-1 w-24 bg-gradient-to-r from-red-500 to-red-700 mx-auto mb-8 rounded-full" />
        <p className="text-base sm:text-lg text-gray-400 max-w-2xl mx-auto mb-12 text-center">{subtitle}</p>
              ...
              <div className="relative z-10 bg-black/90 p-6 sm:p-8 rounded-xl backdrop-blur-md">
                ...
                <div className={`flex flex-col md:flex-row gap-8 ${idx % 2 === 1 ? "md:flex-row-reverse" : ""}`}>
                  <div className="md:w-1/2">
                    ...
                    <div className="mt-4">
                      <h4 className="font-semibold text-red-400 mb-2">Technologies Used</h4>
                      <div className="flex flex-wrap gap-2">
                        {exp.technologies.map((tech: string, i: number) => (
                          <span key={i} className="bg-red-500/20 px-3 py-1 rounded-full text-sm text-gray-200">{tech}</span>
                        ))}
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="md:w-1/2 space-y-4">
                    ...
                    <div className="flex flex-wrap items-center gap-4 text-sm text-gray-400">
                      <span className="flex items-center gap-1"><FaBuilding /> {exp.company}</span>
                      ...
                      <span className="bg-red-500/20 px-2 py-0.5 rounded text-xs">{exp.type}</span>
                    </div>
                    ...
                    <div>
                      <h4 className="font-semibold text-red-400 mb-2">Related Links</h4>
